...
Dr.Sutton's JSON parser that he wrote in class. It will be used to read JSON
syntax from reddit.com and then extract author name and title of posts.
*/
...
indentation indent(int level, bool inl)
{
  return indentation(level, inl);
}

trailer comma(bool c)
{
  return trailer(c);
}

std::ostream& operator<<(std::ostream& os, indentation in)
{
  if (!in.inl)
...
std::ostream& operator<<(std::ostream& os, trailer trail)
{
  if (trail.cont)
    os << ',';
  return os;
